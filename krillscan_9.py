...
class Worker(QtCore.QThread):
    ...
    def read_raw(self,rawfile):       
        ...
        #fix for strange raw files 
        a=raw_obj.raw_data[raw_obj.frequency_map[f][0]]
        if len(a)==  raw_obj.end_ping:
            ...
            for j in range(len(a)):       
                raw_data = raw_obj.raw_data[raw_obj.frequency_map[f][0]][j]       
                sv_obj = raw_data.get_sv()    
                pn =pd.DataFrame(  raw_obj.nmea_data.interpolate(sv_obj, 'GGA')[1] )
                positions = pd.concat([ positions,pn ])

                n_sampl.append( raw_data.sample_count[0] )
                ran.append( sv_obj.range.max() )
            nmax= np.max(n_sampl)
            rangemax= np.max(ran)
            svr = np.ones([nmax,raw_obj.end_ping]) *np.nan
            t=[]    
            ...
            for j in range(len(a)):       
                ...
                svr[0:raw_data.sample_count[0] ,j] = 10*np.log10( sv_obj.data ) 
                t.append(sv_obj.ping_time[0])
            r=np.arange( 0 , rangemax , 0.5 )    
            t=np.array(t) 
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
       ...
       # Thread for emails           
        ...
        self.settingsdefaultbutton =  self.settingsMenu.addAction('Re-set to default')
        self.settingsdefaultbutton.triggered.connect(self.settings_default)   
    ...
    def exploration_mode(self):
        ...
        self.config = configparser.ConfigParser()
        self.config.read('settings.ini') 
        sv_min=float(self.config['GRAPHICS']['sv_min'])
        sv_max=float(self.config['GRAPHICS']['sv_max'])        
        ...
        if len(nasc_done)>0:
            ...
            ix_too_slow = self.df_nasc['speed_knots'] < float(self.config['GRAPHICS']['min_speed_in_knots'])                  
            ...
            gs = GridSpec(2, 2, height_ratios=[4,1],hspace=0.05)

            
            self.canvas.axes1 = self.canvas.fig.add_subplot(gs[0,0])
            ...
            # print(xt)
            img=self.canvas.axes1.imshow( np.rot90( df_sv.values ) , aspect='auto',cmap=self.cmap_ek80,origin = 'lower',vmin=sv_min,vmax=sv_max,extent=xt)        


            cbaxes = inset_axes(self.canvas.axes1, width="100%", height="100%", loc=3,bbox_to_anchor=(.05, .15, .4, .04),bbox_transform=self.canvas.axes1.transAxes) 
            cbar=self.canvas.fig.colorbar(img,cax=cbaxes, orientation='horizontal')
            cbar_ticks = plt.getp(cbar.ax.axes, 'xticklabels')
            plt.setp(cbar_ticks, color='w')
            cbar.ax.xaxis.set_tick_params(color='w')
            cbar.set_label('$s_V$ [dB]', color='w')
            ...
            self.canvas.axes1_3 = self.canvas.fig.add_subplot(gs[1,0] , sharex=self.canvas.axes1)
            self.canvas.axes1_3.set_facecolor('k')   
            self.canvas.axes1_3.grid( alpha=0.5, linestyle='--')
            plt.setp(self.canvas.axes1.get_xticklabels(), visible=False)
            self.canvas.axes1_3.set_xlabel('Time [min]')
            self.canvas.axes1_3.set_ylabel('Krill NASC 1-min avg.')

            self.canvas.axes1.callbacks.connect('xlim_changed', on_xlims_change_echogram)
            self.canvas.axes1_3.callbacks.connect('xlim_changed', on_xlims_change_echogram)
            ...
            ymax=np.max(nasc_swarm)
            if ymax > float(self.config['GRAPHICS']['nasc_graph_max']):
                ymax=float(self.config['GRAPHICS']['nasc_graph_max'])
            self.canvas.axes1_3.set_ylim([0,ymax])     
            ...
            self.canvas.axes4 = self.canvas.fig.add_subplot(gs[:,1],projection=ccrs.Orthographic(central_lon, central_lat))
            self.canvas.axes4.set_facecolor('k')  
            
            # self.canvas.axes4.set_extent(extent)
              

            ixnan=df_krig['nasc'] > float(self.config['GRAPHICS']['nasc_map_max'])     
            ...
            self.canvas.axes4.add_feature(cart.feature.LAND, zorder=100, color='silver')
            ...
            plt.setp(cbar_ticks, color='w')
            cbar.ax.xaxis.set_tick_params(color='w')
            cbar.set_label('10 min avg. Krill NASC', color='w')
            cbar.outline.set_edgecolor('w')
    ...
    def scan_and_vizualize(self):
        ...
        if len(nasc_done)>0:
            ...
            #################################################################new
            # geod = Geod("+ellps=WGS84")
            # fwd_az = geod.inv(df_krig.iloc[0:-1,1],df_krig.iloc[0:-1,0] , df_krig.iloc[1::,1], df_krig.iloc[1::,0] )
            # dif_az=np.concatenate([ np.array([0]) ,np.diff( fwd_az[0] ) , np.array([0])])        
            # ix_not_transect= np.abs(dif_az) > 45
            # df_krig.loc[ix_not_transect,'nasc']=np.nan  
            
            # plt.scatter(df_krig.iloc[0:-2,1],df_krig.iloc[0:-2,0],20,dif_az)
            # plt.show()
            ...
            self.canvas.axes1.grid( alpha=0.5, linestyle='--')
            # self.canvas.axes1.set_xticklabels([])
            ...
            self.canvas.axes1_3.set_xlim([t[0],t[-1]])     
            self.canvas.axes1_3.set_xlabel('Time [min]')
            # self.canvas.axes1_3.xaxis.set_tick_params(labelbottom=True)
            plt.setp(self.canvas.axes1.get_xticklabels(), visible=False)
            ymax=np.max(nasc_swarm)
            if ymax > float(self.config['GRAPHICS']['nasc_graph_max']):
                ymax=float(self.config['GRAPHICS']['nasc_graph_max'])
            ...
            # ixna=df_krig['nasc'].isna()
            # self.canvas.axes4.plot( df_krig.loc[ixna,'lon'],df_krig.loc[ixna,'lat'],'.w',markersize=5,transform=ccrs.PlateCarree() )   
            ...
            # self.scale_bar(self.canvas.axes4, location=(0.5, 0.05),scalecolor='silver')
        #Get the limits of the axis in lat long
            ...
            scalecolor='silver'
            # length=10
            llx0, llx1, lly0, lly1 = self.canvas.axes4.get_extent(ccrs.PlateCarree())
            #Make tmc horizontally centred on the middle of the map,
            #vertically at scale bar location
            sbllx = (llx1 + llx0) / 2
            sblly = lly0 + (lly1 - lly0) * location[1]
            tmc = ccrs.Orthographic(sbllx, sblly)
            #Get the extent of the plotted area in coordinates in metres
            x0, x1, y0, y1 = self.canvas.axes4.get_extent(tmc)
            #Turn the specified scalebar location into coordinates in metres
            sbx = x0 + (x1 - x0) * location[0]
            sby = y0 + (y1 - y0) * location[1]
            length = np.round((x1 - x0)/5000,2)
               
            #Generate the x coordinate for the ends of the scalebar
            bar_xs = [sbx - length * 500, sbx + length * 500]
            #Plot the scalebar
            self.canvas.axes4.plot(bar_xs, [sby, sby], transform=tmc, color=scalecolor, linewidth=3)
            #Plot the scalebar label
            self.canvas.axes4.text(sbx, sby, str(length) + ' km', transform=tmc, color=scalecolor,
